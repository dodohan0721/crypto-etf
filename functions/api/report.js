...
export async function onRequestPost({ request, env }) {
  ...
  const row = await env.DB.prepare(
    "SELECT user_id FROM comments WHERE id = ? AND deleted = 0").bind(id).first();
  if (!row) return json({ error: "not_found" }, 404);
  if (row.user_id === u.id) return json({ error: "self" , msg: "내 글은 신고할 수 없습니다." }, 400);

  // 같은 사람이 같은 글을 여러 번 신고해도 한 건으로 센다.
  await env.DB.prepare(
    `INSERT INTO reports (comment_id, user_id, reason, created) VALUES (?, ?, ?, ?)
  ...
  const c = await env.DB.prepare(
    "SELECT COUNT(*) AS n FROM reports WHERE comment_id = ?").bind(id).first();
  if (c && c.n >= 3) {
    await env.DB.prepare("UPDATE comments SET deleted = 1, del_by = 'reported' WHERE id = ?")
      .bind(id).run();
  }
